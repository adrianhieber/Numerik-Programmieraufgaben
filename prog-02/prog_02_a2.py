...
class compressed_img:
    ...
    def compress_svd(self, I, t):
        for i in range(0,self.coloranzahl):
            ...
            type(self).u=np.resize(type(self).u,(ui.shape[0],ui.shape[1],self.coloranzahl))
            type(self).s=np.resize(type(self).s,(si.shape[0],self.coloranzahl))
            type(self).v=np.resize(type(self).v,(vi.shape[0],vi.shape[1],self.coloranzahl))
    ...
    def __call__(self):
        ...
        for i in range(0,self.coloranzahl):
            ret[:,:,i] = type(self).u[:,:,i] @ np.diag(type(self).s[:,i]) @ type(self).v[:,:,i].T
        ret[ret < 0] = 0
        ret[ret > 1] = 1
        return ret
